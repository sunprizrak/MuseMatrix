...
class Class(with_metaclass(MetaJavaClass, JavaClass)):
    ...
    forName = JavaMultipleMethod(
        [
            (
                "(Ljava/lang/String,Z,Ljava/lang/ClassLoader;)Ljava/langClass;",
                True,
                False,
            ),
    ...
    getDeclaredConstructor = JavaMethod(
    ...
    def __str__(self):
        return ("%s: [%s]" if self.isArray() else "%s: %s") % (
            "Interface"
            if self.isInterface()
            else "Primitive"
            if self.isPrimitive()
...
def autoclass(clsname, include_protected=True, include_private=True):
    jniname = clsname.replace(".", "/")
    cls = MetaJavaClass.get_javaclass(
        jniname, classparams=(include_protected, include_private)
    )
    if cls:
        return cls
    ...
    # log.debug(
    #     "autoclass(%s) intf %r hierarchy is %s"
    #     % (clsname, c.isInterface(), str(class_hierachy))
    # )
    cls_done = set()
    ...
    for cls, level in class_hierachy:
        ...
        for index, method in enumerate(methods):
            ...
            if Modifier.isPrivate(method_modifier) and not include_private:
                continue
            if not (
                Modifier.isPublic(method_modifier)
                or Modifier.isProtected(method_modifier)
                or Modifier.isPrivate(method_modifier)
            ):
    ...
    for field_name, (field, _, cls_packagename) in cls_fields.items():
        ...
        if Modifier.isPrivate(field_modifier) and not include_private:
            continue
        if not (
            Modifier.isPublic(field_modifier)
            or Modifier.isProtected(field_modifier)
            or Modifier.isPrivate(field_modifier)
        ):
            if cls_start_packagename == cls_packagename and not include_protected:
                continue
            if cls_start_packagename != cls_packagename and not include_private:
    ...
    for name in cls_methods:
        if len(cls_methods[name]) == 1:
            ...
            classDict[name] = (JavaStaticMethod if static else JavaMethod)(
                sig, varargs=varargs
            )
            ...
            if (
                ...
                if lowername in classDict:
                    ...
                    continue
                classDict[lowername] = (
                    lambda n: property(lambda self: getattr(self, n)())
                )(name)
        else:
            # multiple signatures
            signatures = []
            # log.debug(
            #     "method %s has %d multiple signatures in hierarchy of cls %s"
            #     % (name, len(cls_methods[name]), c)
            # )
            ...
            for owningCls, method, level in cls_methods[name]:
                param_sig = "".join(
                    [get_signature(x) for x in method.getParameterTypes()]
                )
                # log.debug(
                #     "\t owner %s level %d param_sig %s"
                #     % (str(owningCls), level, param_sig)
                # )
            ...
            for owningCls, method, level in cls_methods[name]:
                ...
                if level > paramsig_to_level[param_sig]:
                    # log.debug(
                    #     "discarding %s name from %s at level %d"
                    #     % (name, str(owningCls), level)
                    # )
                    continue
                ...
                if log.isEnabledFor(DEBUG):
                    log_method(method, name, sig)
                signatures.append(
                    (sig, Modifier.isStatic(method.getModifiers()), method.isVarArgs())
                )

            # log.debug(
            #     "method selected %d multiple signatures of %s"
            #     % (len(signatures), str(signatures))
            # )
...
def _getitem(self, index):
    ...
    try:
        ...
        if find_javaclass("java.lang.IndexOutOfBoundsException").isInstance(
            mock_exception_object
        ):
...
protocol_map = {
    ...
    "java.util.List": {"__getitem__": _getitem},
    "java.util.Map": {
